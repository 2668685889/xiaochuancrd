...
class SalesOrder(Base):
    ...
    customer_uuid = Column(CHAR(36), ForeignKey('customers.uuid'), nullable=False, index=True)
    ...
    status = Column(Enum('PENDING', 'CONFIRMED', 'SHIPPED', 'DELIVERED', 'CANCELLED', 'DELETED'), 
    ...
    def __repr__(self):
        return f"<SalesOrder(number='{self.order_number}', status='{self.status}')>"
...
class SalesOrderItem(Base):
    ...
    uuid = Column(CHAR(36), primary_key=True, default=lambda: str(uuid4()))
    sales_order_uuid = Column(CHAR(36), ForeignKey('sales_orders.uuid'), nullable=False, index=True)
    ...
    def __repr__(self):
        return f"<SalesOrderItem(product='{self.product_uuid}', quantity={self.quantity})>"